CREATE DATABASE FlexTrainer3;
DROP DATABASE FlexTrainer3;
USE FlexTrainer3;
...
(
    SELECT dp.Diet_Plan_ID AS ID,
           ...
           SUM(d.Carbs * dpu.Quantity) AS total_carbs,
           SUM(d.Proteins * dpu.Quantity) AS total_proteins,
           dp.Public_Flag
    FROM Diet_Plan dp
...
-- 1. Details of members of one specific gym that get training from 1 specific trainer.            V
CREATE OR ALTER FUNCTION GetMembersByGymAndTrainer
    (@gym_ssn VARCHAR(11), @trainer_ssn VARCHAR(11))
RETURNS TABLE
AS
RETURN
(
    SELECT m.Member_SSN,m.Gender,m.First_Name + ' ' +m.Last_Name AS Member_Name, m.Gmail,m.Height,m.Weight
    FROM Member m
    JOIN MemberjoinGym mjg ON m.Member_SSN = mjg.Member_SSN
    JOIN MemberTrainerSession mts ON m.Member_SSN = mts.Member_SSN
    WHERE mjg.GYM_SSN = @gym_ssn
    AND mts.Trainer_SSN = @trainer_ssn
);

SELECT DISTINCT * FROM GetMembersByGymAndTrainer('12345678902', 'T_001');

-- Insert more dummy data into MemberjoinGym table
INSERT INTO MemberjoinGym (GYM_SSN, Member_SSN)
VALUES 
('', '11111111111'),
('12345678902', '22222222222'),
('12345678902', '33333333333'),
('12345678902', '44444444444'),
('12345678902', '55555555555'),
('12345678902', '66666666666'),
('12345678902', '77777777777'),
('12345678902', '88888888888'),
('12345678902', '99999999999'),
('12345678902', '10101010101');

SELECT  * FROM MemberTrainerSession


-- 2. Details of members from one specific gym that follow a specific diet plan.                     X 
CREATE OR ALTER FUNCTION GetMembersByGymAndDietPlan
    (@gym_ssn VARCHAR(11), @diet_plan_id INT)
RETURNS TABLE
AS
RETURN
(
    ...
    FROM Member m
    JOIN MemberjoinGym mjg ON m.Member_SSN = mjg.Member_SSN
    JOIN WeeklyPlan wp ON m.Member_SSN = wp.Member_SSN
    WHERE mjg.GYM_SSN = @gym_ssn
    AND wp.Diet_Plan_ID = @diet_plan_id
);

SELECT DISTINCT * FROM  GetMembersByGymAndDietPlan('12345678902', 1);


SELECT * FROM GetMembersByGymAndDietPlan('11111111111', 1);

-- 3. Details of members across all gyms of a specific trainer that follow a specific diet plan.       X
CREATE OR ALTER FUNCTION GetMembersByTrainerAndDietPlan
    (@Trainer_SSN VARCHAR(11), @Diet_Plan_ID INT)
RETURNS TABLE
AS
RETURN
(
    ...
    JOIN MemberTrainerSession mts ON m.Member_SSN = mts.Member_SSN
    JOIN MemberjoinGym mjg ON m.Member_SSN = mjg.Member_SSN
    WHERE mts.Trainer_SSN = @Trainer_SSN
    AND wp.Diet_Plan_ID = @Diet_Plan_ID
);

SELECT DISTINCT * FROM GetMembersByTrainerAndDietPlan('T_001', 2);

-- 4. Count of members who will be using specific machines on a given day in a specific gym.           X
-- 5. List of Diet plans that have less than 500 calorie meals as breakfast.                           V
-- 6. List of diet plans in which total carbohydrate intake is less than 300 grams.                    V
-- 7. List of workout plans that don’t require using a specific machine.                               X
-- 8. List of diet plans which doesn’t have peanuts as allergens.                                      X                       
-- 9. New membership data in last 3 months (Gym Owner). 
CREATE OR ALTER FUNCTION GetNewMembershipsByGym
    (@gym_ssn VARCHAR(11))
RETURNS TABLE
AS
RETURN
(
    SELECT *
    FROM Membership
    WHERE GYM_SSN = @gym_ssn
        AND Join_Date >= DATEADD(MONTH, -3, GETDATE())
);

SELECT * FROM GetNewMembershipsByGym('GYM_005');
-- 10. Comparison of total members in multiple gyms, in the past 6 months.                             X
-- return total members of gym
CREATE OR ALTER FUNCTION GetTotalMembersByGym()
RETURNS TABLE
AS
RETURN
(
    SELECT g.Name, COUNT(*) AS Total_Members
    FROM MemberjoinGym mjg
    INNER JOIN Gym g ON mjg.GYM_SSN = g.GYM_SSN
    GROUP BY g.Name
);

SELECT * FROM GetTotalMembersByGym();


CREATE OR ALTER PROCEDURE InsertGymLog
AS
BEGIN
    INSERT INTO GymLog (GYM_Name, TotalMembers)
    SELECT * FROM GetTotalMembersByGym();
END;
...
-- 2. Details of members from one specific gym that follow a specific diet plan.                                  V
SELECT DISTINCT * FROM GetMembersByGymAndDietPlan('11111111111', 1);  
SELECT GYM_SSN FROM Gym;
SELECT Diet_Plan_ID FROM Diet_Plan;
-- 3. Details of members across all gyms of a specific trainer that follow a specific diet plan.                  V
SELECT DISTINCT * FROM GetMembersByTrainerAndDietPlan('T_001', 2);
SELECT Trainer_SSN FROM Trainer;
SELECT Diet_Plan_ID FROM Diet_Plan;
-- 4. Count of members who will be using specific machines on a given day in a specific gym.                      X
...
-- 7. List of workout plans that don’t require using a specific machine.                                          X
-- 8. List of diet plans which doesn’t have peanuts as allergens.                                                 X
-- 9. New membership data in last 3 months (Gym Owner).                                                           O
SELECT * FROM GetNewMembershipsByGym('GYM_005');
-- 10. Comparison of total members in multiple gyms, in the past 6 months.                                        O
SELECT GYM_Name, AVG(TotalMembers) FROM GymLog WHERE LogDate >= DATEADD(MONTH, -3, GETDATE()) GROUP BY GYM_Name;